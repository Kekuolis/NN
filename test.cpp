...
int main(int argc, char **argv) {
  ...
  cerr << "Reading training data from " << argv[1] << "...\n";
  {
    ...
    cerr << tlc << " lines, " << sd.size() << " source types, " << td.size()
         << " target types\n";
  }
  ...
  while (1) {
    ...
    for (unsigned i = 0; i < report_every_i; ++i) {
      ...
      Expression sim =
          squared_distance(s, emb.EmbedTarget(sent_pair.second, cg));
      float margin = 2;
      const unsigned K = 20;
      vector<Expression> noise(K);
      for (unsigned j = 0; j < K; ++j) {
        unsigned sample = rand01() * training.size();
        while (sample == order[si] || sample == training.size()) {
          sample = rand01() * training.size();
        }
        Expression sim_n =
            squared_distance(s, emb.EmbedTarget(training[sample].second, cg));
